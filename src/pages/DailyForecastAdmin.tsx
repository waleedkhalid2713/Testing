...
import { calculateTradeMetrics, getTradeWarnings, resolveTradingViewSymbol, validateForecastDraft, type TradeDirection } from "@/lib/tradeForecast";
...
type FormState = { instrumentId: string; sourceType: SourceType; tradingViewSymbol: string; exchange: string; timeframe: string; direction: TradeDirection; executionPrice: string; stopLoss: string; takeProfit1: string; takeProfit2: string; takeProfit3: string; expectedPnl: string; status: TradeStatus; tradeDate: string; rationale: string; notes: string; resultPnl: string; resultPnlPercent: string; resultNotes: string };
const today = () => new Date().toISOString().slice(0, 10);
const emptyForm = (): FormState => ({ instrumentId: "", sourceType: "screenshot", tradingViewSymbol: "", exchange: "", timeframe: "60", direction: "long", executionPrice: "", stopLoss: "", takeProfit1: "", takeProfit2: "", takeProfit3: "", expectedPnl: "", status: "active", tradeDate: today(), rationale: "", notes: "", resultPnl: "", resultPnlPercent: "", resultNotes: "" });
...
export default function DailyForecastAdmin() {
  ...
  const [loadedTradingViewSymbol, setLoadedTradingViewSymbol] = useState("");
  const [chartLoadedAt, setChartLoadedAt] = useState<string | null>(null);
  const [symbolError, setSymbolError] = useState("");
  const [legacySymbolWarning, setLegacySymbolWarning] = useState(false);
  const [listStatus, setListStatus] = useState("");
  const [listSource, setListSource] = useState("");
  const [listQuery, setListQuery] = useState("");
  const selectedInstrument = instruments.find((item) => item.id === form.instrumentId);
  const resolvedFormSymbol = resolveTradingViewSymbol(form.tradingViewSymbol);
  ...
  const generateNotes = async (result = false) => {
    ...
    try {
      ...
      const { data, error: fnError } = await supabase.functions.invoke("analyze-trade-image", { body: { mode: "notes", image, trade: { symbol: resolvedFormSymbol.ok ? resolvedFormSymbol.symbol : selectedInstrument.symbol, exchange: resolvedFormSymbol.ok ? resolvedFormSymbol.exchange : form.exchange, timeframe: form.timeframe, direction: form.direction, entry: prices.entry, stopLoss: prices.stop, takeProfits: targets, riskReward: metrics.riskRewardRatio, expectedReturnPercent: metrics.rewardPercent, expectedPnl: numberOrNull(form.expectedPnl), rationale: form.rationale, result: result ? { status: form.status, pnl: numberOrNull(form.resultPnl), pnlPercent: numberOrNull(form.resultPnlPercent), notes: form.resultNotes } : null } } });
  ...
  const reset = () => { setForm(emptyForm()); setEditing(null); setPreFile(null); setPostFile(null); setPrePreview(null); setPostPreview(null); setExtraction(null); setLoadedTradingViewSymbol(""); setChartLoadedAt(null); setSymbolError(""); setLegacySymbolWarning(false); setError(""); };
  const editForecast = (forecast: Forecast) => {
    const instrument = instruments.find((item) => item.id === forecast.instrument_id);
    // Legacy rows had no authoritative full symbol; combine existing fields for review only.
    const legacyDisplaySymbol = forecast.exchange && instrument?.symbol ? `${forecast.exchange}:${instrument.symbol.replace(/\//g, "")}` : "";
    const tradingViewSymbol = forecast.tradingview_symbol ?? legacyDisplaySymbol;
    const resolved = resolveTradingViewSymbol(tradingViewSymbol);
    setEditing(forecast); setForm({ instrumentId: forecast.instrument_id, sourceType: forecast.source_type as SourceType, tradingViewSymbol, exchange: forecast.exchange, timeframe: forecast.timeframe, direction: forecast.direction as TradeDirection, executionPrice: String(forecast.execution_price), stopLoss: String(forecast.stop_loss), takeProfit1: String(forecast.take_profit_1), takeProfit2: forecast.take_profit_2?.toString() ?? "", takeProfit3: forecast.take_profit_3?.toString() ?? "", expectedPnl: forecast.expected_pnl?.toString() ?? "", status: forecast.status as TradeStatus, tradeDate: forecast.trade_date, rationale: forecast.rationale, notes: forecast.notes, resultPnl: forecast.result_pnl?.toString() ?? "", resultPnlPercent: forecast.result_pnl_percent?.toString() ?? "", resultNotes: forecast.result_notes });
    setLoadedTradingViewSymbol(forecast.tradingview_symbol && resolved.ok ? resolved.symbol : "");
    setChartLoadedAt(typeof (forecast.chart_metadata as { loadedAt?: unknown } | null)?.loadedAt === "string" ? (forecast.chart_metadata as { loadedAt: string }).loadedAt : null);
    setLegacySymbolWarning(Boolean(!forecast.tradingview_symbol && tradingViewSymbol));
    setSymbolError("");
  ...
  const loadChart = () => {
    const resolved = resolveTradingViewSymbol(form.tradingViewSymbol);
    if (resolved.ok === false) { setSymbolError(resolved.error); return; }
    setForm((current) => ({ ...current, tradingViewSymbol: resolved.symbol, exchange: resolved.exchange }));
    setLoadedTradingViewSymbol(resolved.symbol);
    setChartLoadedAt(new Date().toISOString());
    setLegacySymbolWarning(false);
    setSymbolError("");
  };
  ...
  const save = async (event: FormEvent) => {
    ...
    const validationErrors = validateForecastDraft({
      ...
      tradingViewSymbol: form.tradingViewSymbol,
      loadedTradingViewSymbol,
    ...
    try {
      ...
      const resolvedSymbol = resolveTradingViewSymbol(form.tradingViewSymbol);
      const confirmedSymbol = resolvedSymbol.ok ? resolvedSymbol : null;
      const payload = { instrument_id: form.instrumentId, source_type: form.sourceType, tradingview_symbol: form.sourceType === "live_chart" ? confirmedSymbol?.symbol ?? null : editing?.tradingview_symbol ?? null, exchange: form.sourceType === "live_chart" ? confirmedSymbol?.exchange ?? "" : form.exchange.trim(), timeframe: form.timeframe, direction: form.direction, execution_price: prices.entry, stop_loss: prices.stop, take_profit_1: prices.target, take_profit_2: numberOrNull(form.takeProfit2), take_profit_3: numberOrNull(form.takeProfit3), expected_pnl: numberOrNull(form.expectedPnl), status: form.status, trade_date: form.tradeDate, rationale: form.rationale.trim(), notes: form.notes.trim(), result_pnl: numberOrNull(form.resultPnl), result_pnl_percent: numberOrNull(form.resultPnlPercent), result_notes: form.resultNotes.trim(), image_path: imagePath, result_image_path: resultPath, ai_extraction: extraction, chart_metadata: form.sourceType === "live_chart" && confirmedSymbol ? { provider: "TradingView hosted chart widget", symbol: confirmedSymbol.symbol, exchange: confirmedSymbol.exchange, instrument: confirmedSymbol.instrument, timeframe: form.timeframe, loadedAt: chartLoadedAt, containsDrawings: false } : null, updated_at: new Date().toISOString() };
  ...
  return <div><PageHero title="Trade Forecast Admin" subtitle="Publish screenshot-based forecasts or build a setup alongside a live TradingView chart." imageSrc={heroImage} imageAlt="Forecast dashboard with candlestick charts" />
    <div className="container space-y-8 py-12"><Card><CardHeader><CardTitle>{editing ? "Edit forecast" : "Create forecast"}</CardTitle><CardDescription>Both workflows use the same validated trade model, evidence storage, and public forecast display.</CardDescription></CardHeader><CardContent>
      <form className="space-y-6" onSubmit={save}>
        ...
        <div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-sm font-medium">Trading pair<select className="h-10 w-full rounded-md border bg-background px-3" value={form.instrumentId} onChange={(e) => setField("instrumentId", e.target.value)}><option value="">Choose instrument</option>{marketGroups.map((group) => <optgroup key={group} label={group}>{instruments.filter((item) => `${item.market_type} · ${item.sub_market}` === group).map((item) => <option key={item.id} value={item.id}>{item.symbol} — {item.name}</option>)}</optgroup>)}</select></label><label className="space-y-2 text-sm font-medium">Trade date<Input type="date" value={form.tradeDate} onChange={(e) => setField("tradeDate", e.target.value)} /></label></div>
        {form.sourceType === "live_chart" ? <section className="space-y-4 rounded-xl border p-4" aria-labelledby="tradingview-chart-heading"><h2 id="tradingview-chart-heading" className="text-lg font-semibold">TradingView Chart</h2><div className="grid gap-4 md:grid-cols-[1fr_14rem]"><label className="space-y-2 text-sm font-medium">TradingView Symbol<Input value={form.tradingViewSymbol} maxLength={120} onChange={(event) => { setField("tradingViewSymbol", event.target.value); setSymbolError(""); }} placeholder="EXCHANGE:INSTRUMENT" /><span className="block text-xs font-normal text-muted-foreground">Examples: BINANCE:BTCUSDT · OANDA:XAUUSD · COMEX:GC1! · NASDAQ:AAPL</span></label><label className="space-y-2 text-sm font-medium">Timeframe<select className="h-10 w-full rounded-md border bg-background px-3" value={form.timeframe} onChange={(e) => setField("timeframe", e.target.value)}>{[["1","1 minute"],["5","5 minutes"],["15","15 minutes"],["60","1 hour"],["240","4 hours"],["D","Daily"],["W","Weekly"]].map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><Button type="button" variant="secondary" onClick={loadChart}>Load Chart</Button>{symbolError ? <p role="alert" className="text-sm text-destructive">{symbolError}</p> : null}{legacySymbolWarning ? <Alert><AlertTitle>Previously saved symbol</AlertTitle><AlertDescription>This previously saved TradingView symbol may be unsupported. Confirm or replace it before updating the forecast.</AlertDescription></Alert> : null}<p className="text-sm"><span className="text-muted-foreground">Resolved symbol: </span><strong>{loadedTradingViewSymbol || "Not loaded"}</strong></p>{loadedTradingViewSymbol ? <TradingViewChart symbol={loadedTradingViewSymbol} timeframe={form.timeframe} /> : <div className="flex min-h-72 items-center justify-center rounded-xl border border-dashed text-sm text-muted-foreground">Enter a complete TradingView symbol and select Load Chart.</div>}</section> : null}
